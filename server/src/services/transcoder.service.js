  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const job = {
        ...
        outputUri,
        config: {
          // 🔥 FIX 1: Inputs aur EditList wapas lagaya (HLS mapping ke liye zaroori hota hai)
          ...
          elementaryStreams: [
            {
              ...
              videoStream: {
                h264: {
                  ...
                  frameRate: 30,
                  allowOpenGop: false,
                  // 🔥 FIX 2: gopFrameCount HATA DIYA aur strict gopDuration lagaya
                  gopDuration: { 
                    seconds: 2, 
                    nanos: 0 
                  },
          ...
          muxStreams: [
            {
              key: 'hls-video',
              // 🔥 FIX 3: fileName WAPAS LAGAYA (Iske bina GCP chunks nahi banata)
              fileName: 'chunk_%05d.ts', 
              container: 'ts',
              elementaryStreams: ['video-stream0', 'audio-stream0'],
              segmentSettings: {
                // 🔥 Exact 2 seconds Duration Object
                segmentDuration: { 
                  seconds: 2,
                  nanos: 0 
                }
